...
D=3 #core dimension without pose
latentSize=D
...
with periodGraph.as_default():
	...
	with tf.device('/device:GPU:'+str(gpuID)):
		...
		boundarySize=1000	#from 2D to 3D, so 100 to 1000
		...
		#dis_vars = [var for var in all_vars if var.name.startswith('dis_')]


		optimizer1 = tf.train.AdamOptimizer()
		trainer = optimizer1.minimize(point_rec_loss+bound_rec_loss+out_loss+shrink_loss+0*collapse_loss, var_list=enc_vars+dec_vars)
